...
$PYTHON scripts/run_patchprep_from_config.py  $CFG/patchprep_control.yaml

echo "--- [2/2] ycomp patchprep ---"
$PYTHON scripts/run_patchprep_from_config.py  $CFG/patchprep_ycomp.yaml
...
$PYTHON scripts/run_ae_apply_from_config.py   $CFG/ae_apply_baseline.yaml

echo "--- [2/4] semisup_fa AE apply ---"
$PYTHON scripts/run_ae_apply_from_config.py   $CFG/ae_apply_semisup_fa.yaml

echo "--- [3/4] semisup_pos AE apply ---"
$PYTHON scripts/run_ae_apply_from_config.py   $CFG/ae_apply_semisup_pos.yaml

echo "--- [4/4] semisup_both AE apply ---"
$PYTHON scripts/run_ae_apply_from_config.py   $CFG/ae_apply_semisup_both.yaml
...
$PYTHON scripts/run_cls_apply_from_config.py  $CFG/cls_apply_baseline_fa_lat8.yaml

echo "--- baseline | Position | lat8 ---"
$PYTHON scripts/run_cls_apply_from_config.py  $CFG/cls_apply_baseline_pos_lat8.yaml

echo "--- baseline | FA type  | lat8+dist8 ---"
$PYTHON scripts/run_cls_apply_from_config.py  $CFG/cls_apply_baseline_fa_lat8dist8.yaml

echo "--- baseline | Position | lat8+dist8 ---"
$PYTHON scripts/run_cls_apply_from_config.py  $CFG/cls_apply_baseline_pos_lat8dist8.yaml

# ── semisup_fa ────────────────────────────────────────────────────────────
echo "--- semisup_fa | FA type  | lat8 ---"
$PYTHON scripts/run_cls_apply_from_config.py  $CFG/cls_apply_semisup_fa_fa_lat8.yaml

echo "--- semisup_fa | Position | lat8 ---"
$PYTHON scripts/run_cls_apply_from_config.py  $CFG/cls_apply_semisup_fa_pos_lat8.yaml

echo "--- semisup_fa | FA type  | lat8+dist8 ---"
$PYTHON scripts/run_cls_apply_from_config.py  $CFG/cls_apply_semisup_fa_fa_lat8dist8.yaml

echo "--- semisup_fa | Position | lat8+dist8 ---"
$PYTHON scripts/run_cls_apply_from_config.py  $CFG/cls_apply_semisup_fa_pos_lat8dist8.yaml

# ── semisup_pos ───────────────────────────────────────────────────────────
echo "--- semisup_pos | FA type  | lat8 ---"
$PYTHON scripts/run_cls_apply_from_config.py  $CFG/cls_apply_semisup_pos_fa_lat8.yaml

echo "--- semisup_pos | Position | lat8 ---"
$PYTHON scripts/run_cls_apply_from_config.py  $CFG/cls_apply_semisup_pos_pos_lat8.yaml

echo "--- semisup_pos | FA type  | lat8+dist8 ---"
$PYTHON scripts/run_cls_apply_from_config.py  $CFG/cls_apply_semisup_pos_fa_lat8dist8.yaml

echo "--- semisup_pos | Position | lat8+dist8 ---"
$PYTHON scripts/run_cls_apply_from_config.py  $CFG/cls_apply_semisup_pos_pos_lat8dist8.yaml

# ── semisup_both ──────────────────────────────────────────────────────────
echo "--- semisup_both | FA type  | lat8 ---"
$PYTHON scripts/run_cls_apply_from_config.py  $CFG/cls_apply_semisup_both_fa_lat8.yaml

echo "--- semisup_both | Position | lat8 ---"
$PYTHON scripts/run_cls_apply_from_config.py  $CFG/cls_apply_semisup_both_pos_lat8.yaml

echo "--- semisup_both | FA type  | lat8+dist8 ---"
$PYTHON scripts/run_cls_apply_from_config.py  $CFG/cls_apply_semisup_both_fa_lat8dist8.yaml

echo "--- semisup_both | Position | lat8+dist8 ---"
$PYTHON scripts/run_cls_apply_from_config.py  $CFG/cls_apply_semisup_both_pos_lat8dist8.yaml
