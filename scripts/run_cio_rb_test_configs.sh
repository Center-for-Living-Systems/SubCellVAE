#!/usr/bin/env bash
# =============================================================================
# run_cio_rb_test_configs.sh
# Full pipeline: patchprep → 8 AEs → 8 analyses → 32 classifications → 16 vis
# Normalization: cell_insideoutside + rolling ball r=20  (cio_rb patches)
# Variants: baseline, semisup_{fa,pos,both}, conae, semicon_{fa,pos,both}
#
# Skip patchprep if patches already exist:
#   SKIP_PATCHPREP=1 bash scripts/run_cio_rb_test_configs.sh
#
# Results land in a timestamped directory, e.g.:
#   ae_results/test_run_cio_rb_26042416/{baseline,...}/
#
# Usage:
#   cd /home/lding/lding/gitcode/SubCellAE
#   bash scripts/run_cio_rb_test_configs.sh
#   bash scripts/run_cio_rb_test_configs.sh 2>&1 | tee logs/test_run_cio_rb.log
# =============================================================================
...
# Timestamp: YYMMDDhh  (e.g. 26042416 for 2026-04-24 16:xx)
TS=$(date +%y%m%d%H)
RUN_NAME="test_run_cio_rb_${TS}"
...
TMP_CFG=$(mktemp -d)
for f in config/test_config_cio_rb/*.yaml; do
    sed "s|test_run_cio_rb|$RUN_NAME|g" "$f" > "$TMP_CFG/$(basename $f)"
done
CFG="$TMP_CFG"
...
echo "======================================================================"
echo " STAGE 0 — Patch preparation  (cell_insideoutside + rolling ball)"
echo "======================================================================"
...
if [ "${SKIP_PATCHPREP:-0}" = "1" ]; then
    ...
    $PYTHON scripts/run_patchprep_from_config.py config/patchprep_config/vinc_control_cio_rb.yaml

    echo "--- [2/2] ycomp patchprep ---"
    $PYTHON scripts/run_patchprep_from_config.py config/patchprep_config/vinc_ycomp_cio_rb.yaml
fi
