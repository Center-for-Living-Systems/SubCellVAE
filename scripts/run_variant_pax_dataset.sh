#!/usr/bin/env bash
# =============================================================================
# run_variant_pax_dataset.sh
# Apply dataset-normalization-trained models to all variant-paxillin datasets.
#
...
#   bash scripts/run_variant_pax_dataset.sh <MODEL_RUN> [variant1 variant2 ...]
#   bash scripts/run_variant_pax_dataset.sh test_run_20260322
#   bash scripts/run_variant_pax_dataset.sh test_run_20260322 baseline conae
#   bash scripts/run_variant_pax_dataset.sh test_run_20260322 2>&1 | tee logs/variant_pax_dataset.log
#
# Skip patchprep if patches already exist:
#   SKIP_PATCHPREP=1 bash scripts/run_variant_pax_dataset.sh test_run_20260322
# =============================================================================
...
if [ $# -lt 1 ]; then
    ...
    echo "  MODEL_RUN: training run directory name, e.g. test_run_20260322"
...
PYTHON="python"
CFG_DIR="config/variant_pax_dataset_config"
...
echo ""
echo "======================================================================"
echo " STAGE 0 — Patch preparation (dataset norm, 4 × 2 = 8 runs)"
echo "======================================================================"
...
echo ""
echo "======================================================================"
echo " STAGE 3 — Classifier apply  (${#VARIANTS[@]} variants × ${#DATASETS[@]} datasets × 2 targets)"
echo "======================================================================"
...
echo ""
echo "======================================================================"
echo " STAGE 4 — Cross-classification visualization  (${#VARIANTS[@]} variants × ${#DATASETS[@]} datasets)"
echo "======================================================================"
...
echo ""
echo "======================================================================"
echo " STAGE 5 — Overlay  (${#VARIANTS[@]} variants × ${#DATASETS[@]} datasets × 2 targets)"
echo "======================================================================"
...
for VARIANT in "${VARIANTS[@]}"; do
    for DS in "${DATASETS[@]}"; do
        echo "   Results   : $APPLY_BASE/$VARIANT/$DS/"
    done
done
echo "======================================================================"
echo " ALL DONE"
